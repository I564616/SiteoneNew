...
public class DefaultBruteForceAttackCounter implements BruteForceAttackCounter
{
	...
	public void registerLoginFailure(final String userUid)
	{
		if (StringUtils.isNotEmpty(userUid))
		{
			...
			{
				LOG.debug("Failed Login for user " + Sanitizer.sanitize(userUid) + ", count now " + count.getCounter());
			}
		}
	}
	...
	{
		return StringUtils.lowerCase(getUserIdDecorationService().decorateUserId(userUid));
	}
	...
	{
		return userIdDecorationService;
	}

	public void setUserIdDecorationService(final UserIdDecorationService userIdDecorationService)
	{
