 ...
 */
package com.siteone.pimintegration.persistence.handler;
...
import de.hybris.platform.integrationservices.util.IntegrationItemLogger;
 ...
 */
public class SiteOnePersistenceHandler implements ODataProcessorHandler<PersistenceParam, ODataResponse>
{
	private static final Logger LOG = Log.getLogger(SiteOnePersistenceHandler.class);
	private static final EntityProviderReadProperties READ_CFG = EntityProviderReadProperties.init().mergeSemantic(false).build();
	...
	public ODataResponse handle(final PersistenceParam param) throws ODataException
	{
		...
		IntegrationItemLogger.logItem(storageRequest.getIntegrationItem());
	 ...
	 * @throws EdmException
	 *            is thrown when there's a failure in the conversion
	 */
			...
			throws EdmException
	{
		...
		if (e.getMessageReference().getKey().equals(EntityProviderException.ILLEGAL_ARGUMENT.getKey()))
		{
	 ...
	 *           Contains information on how to persist the request payload
	 * @return The persisted {@link ODataEntry}
	 * @throws EdmException
	 *            is thrown if there's a failure in persistence
	 */
	protected ODataEntry persist(final StorageRequest storageRequest) throws EdmException
	{
		...
		catch (final IntegrationAttributeException | PersistenceRuntimeApplicationException | ItemNotFoundException
				| ItemSearchRequestValidationException | TypeAccessPermissionException | NonUniqueItemFoundException
				| InstanceCreationOfAbstractTypeException e)
		{
			throw e;
		}
		catch (final RuntimeException e)
		{
			...
			throw new PersistenceErrorRuntimeException(e, storageRequest.getIntegrationKey() + ": " + e);
		}
	}
	 ...
	 *           Parameters from the request
	 * @param ctx
	 *           Storage request contains the information on how to create the response
	 * @param persistedEntry
	 *           The persisted {@link ODataEntry}
	 * @return The {@link ODataResponse}
	 * @throws ODataException
	 *            is thrown if a failure occurs creating the response
	 */
			...
			final ODataEntry persistedEntry) throws ODataException
	{
		...
		return getODataResponseBuilder(writeEntry(param, persistedEntry, writeProperties)).header(HttpHeaders.CONTENT_LANGUAGE,
				ctx.getAcceptLocale().toString());
	}
	...
	{
		return EntityProviderWriteProperties.serviceRoot(context.getPathInfo().getServiceRoot()).build();
	}
