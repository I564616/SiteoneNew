 ...
 */
public class SiteOnePunchOutSetupRequestCartProcessing extends PunchOutSetupRequestCartProcessing
{
	private static final Logger LOG = LoggerFactory.getLogger(SiteOnePunchOutSetupRequestCartProcessing.class);
	...
	public void processCartData(final CXML input)
	{
		...
		/*
		 * final CartModel cartModel = getCartService().getSessionCart(); if(cartModel!=null) {
	...
	protected void processItemOutAndAddToSessionCart(final List<ItemOut> itemsOut)
	{
		...
		final List<AbstractOrderEntryModel> cartEntryModels = itemsOut.stream()
				.map(itemOut -> getItemOutConverter().convert(itemOut)).collect(Collectors.toList());
