...
import de.hybris.platform.b2b.punchout.actions.inbound.DefaultPunchOutOrderRequestProcessor;
import de.hybris.platform.b2b.punchout.actions.inbound.PlacePurchaseOrderProcessing;
import de.hybris.platform.b2b.punchout.actions.inbound.PopulateCartPurchaseOrderProcessing;
import de.hybris.platform.b2b.punchout.actions.inbound.PrepareCartPurchaseOrderProcessing;
 ...
 */
public class DefaultSiteOnePunchOutOrderRequestProcessor extends DefaultPunchOutOrderRequestProcessor
{
	private static final Logger LOG = LoggerFactory.getLogger(DefaultSiteOnePunchOutOrderRequestProcessor.class);
	...
	public CXML generatecXML(final CXML requestBody)
	{
		...
		if (cartCode != null)
		{
			String cartCodeValue=cartCode.replace("|", "");
			cartModel = restoreSessionCartByCode(cartCodeValue);
			LOG.info(String.format("restored punchOut cart to session. cartId=%s", cartCodeValue));
		}
		...
		if (cartModel == null)
		{
			...
			LOG.info("create a new punchOut cart according to the request cxml");
		}
		...
		return CXMLBuilder.newInstance().withResponseCode(HttpStatus.OK).withResponseMessage(PunchOutResponseMessage.OK)
				.withResponseValue(orderData.getCode()).create();
	}
	...
	protected CartModel restoreSessionCartByCode(final String cartCode)
	{
		...
		LOG.info(String.format("get punchout cart cartId=%s, userId=%s", cartCode, user.getUid()));
		final CartModel cartModel = getCommerceCartService().getCartForCodeAndUser(cartCode, user);
		if (cartModel == null)
		{
			LOG.info(String.format("get punchout cart failed. cartId=%s, userId=%s", cartCode, user.getUid()));
	...
	protected String getRequestCartCode(final CXML requestBody)
	{
		...
		{
			final SupplierPartAuxiliaryID supplierPartAuxiliaryID = itemOut.getItemID().getSupplierPartAuxiliaryID();
			if (supplierPartAuxiliaryID != null && !supplierPartAuxiliaryID.getContent().isEmpty())
			{
				final String curItemCartCode = (String) supplierPartAuxiliaryID.getContent().get(0);
	...
	protected boolean validateCart(final CXML requestBody, final CartModel cartModel)
	{
		...
		final Map<String, AbstractOrderEntryModel> cartEntryMap = cartEntries.stream()
				.collect(Collectors.toMap((cartEntryModel -> cartEntryModel.getProduct().getCode()), Function.identity()));
	...
	/**
	 * @param prepareCartPurchaseOrderProcessing
	 *                                              the prepareCartPurchaseOrderProcessing to set
	 */
	...
	/**
	 * @param populateCartPurchaseOrderProcessing
	 *                                               the populateCartPurchaseOrderProcessing to set
	 */
	...
	/**
	 * @param placePurchaseOrderProcessing
	 *                                        the placePurchaseOrderProcessing to set
	 */
