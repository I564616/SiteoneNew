 ...
 *
 */
public class DefaultSiteOneB2BApprovalBusinessProcessStrategy implements BusinessProcessStrategy
{
	...
	public void createB2BBusinessProcess(final OrderModel order)
	{
		...
		final String approvalProcessCode = (StringUtils.isNotBlank(parentUnit.getApprovalProcessCode())
				? parentUnit.getApprovalProcessCode()
				: getProcessCode());
		...
		final B2BApprovalProcessModel approvalProcess = (B2BApprovalProcessModel) this.getBusinessProcessService().createProcess(
				String.valueOf("b2bOrderApproval-process-" + getProcessCodeGenerator().generate()), approvalProcessCode,
				contextParams);
