 ...
 *
 */
package com.siteone.core.content.search.service;
 ...
 *         implementation is stateless, i.e. it does not maintain any state for each search, instead it externalizes the
 *         search state in the search page data returned. The search must be initiated by calling either of the 2 search
 *         methods {@link #textSearch(String,PageableData)} or {@link #categorySearch(String,PageableData)}. From these
 *         you get the search page data result {@link FacetSearchPageData}. The search page data includes state
 *         instances for each search refinement option, and to perform the refinement the
 *         {@link #searchAgain(Object, de.hybris.platform.commerceservices.search.pagedata.PageableData)} method must be
 *         called passing back in the appropriate state instance. The state instance is a memento representing the
 *         search state, it is an opaque instance that only has meaning to the search service implementation. Callers
 *         should not create their own instances or modify the instances returned.
 *
 * @param <STATE>
 ...
 */
public interface SolrContentSearchService<STATE, ITEM, RESULT extends ContentSearchPageData<STATE, ITEM>>
{
	/**
