 ...
 *
 */
public class ContentSearchCrawledContentValueProvider extends AbstractPropertyFieldValueProvider
		implements Serializable, FieldValueProvider
{
	...
	@Resource(name = "contentCrawler")
	private transient ContentCrawler contentCrawler;
			...
			final Object model) throws FieldValueProviderException
	{
		...
		final Map<String, String> crawledContent;
		...
		if (model instanceof ContentPageModel)
		{
			...
			{
				crawledContent = contentCrawler.getCrawledContentForUrl(url);
			}
			else
			{
				crawledContent = contentCrawler.getCrawledContentForUrl("/" + url);
			...
			addFieldValues(fieldValues, indexedProperty, crawledContent.get(CONTENT));
		}
		return fieldValues;
	}
