 ...
 *
 */
public class SDSMediaValueProvider extends AbstractPropertyFieldValueProvider implements FieldValueProvider, Serializable
{
			...
			final Object model) throws FieldValueProviderException
	{
		...
		if (CollectionUtils.isNotEmpty(dataSheets))
		{
			...
			{
				if (StringUtils.isNotBlank(media.getSds()))
				{
	...
	private List<FieldValue> createFieldValue(final MediaModel sdsMedia, final IndexedProperty indexedProperty)
	{
		...
		{
			fieldValues.add(new FieldValue(fieldName, sdsMedia.getSds() + "||" + sdsMedia.getURL()));
		}
		return fieldValues;
	}
