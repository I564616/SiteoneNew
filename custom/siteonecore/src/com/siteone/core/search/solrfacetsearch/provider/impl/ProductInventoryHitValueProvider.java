 ...
 *
 */
public class ProductInventoryHitValueProvider extends AbstractPropertyFieldValueProvider
		implements FieldValueProvider, Serializable
{
			...
			else
			{
				for (final VariantProductModel variantProductModel : variants)
				{
					...
					if (CollectionUtils.isNotEmpty(variantsPointOfServiceList))
					{
						...
						{
							isSellable = isSellableInventoryHit(variantProductModel, pointOfServiceModel);
							if (isSellable)
							{
	...
	protected List<FieldValue> createFieldValueForVariantProduct(final boolean isSellable,
			final PointOfServiceModel pointOfService, final IndexedProperty indexedProperty, final String storeId)
	{
		...
		{
			addFieldValues(fieldValues, indexedProperty, Boolean.valueOf(isSellable), storeId);
		}
		return fieldValues;
	}
	...
	protected boolean isSellableInventoryHit(final ProductModel product, final PointOfServiceModel pointOfService)
	{
		...
		{
			final boolean forceInStock = BooleanUtils.isTrue(((List<StockLevelModel>) stockLevelList).get(0).getForceInStock())
					&& (BooleanUtils.isNotTrue(product.getInventoryCheck())
							|| BooleanUtils.isNotTrue(((List<StockLevelModel>) stockLevelList).get(0).getRespectInventory()));
			inventoryHitCount = (((List<StockLevelModel>) stockLevelList).get(0).getQuantityLevel());
			stockLevel = getCommerceStockService().getStockLevelForProductAndPointOfService(product, pointOfService);
			if ((null != stockLevel && stockLevel <= Long.valueOf(0) && !forceInStock)
					&& ((null != inventoryHitCount && inventoryHitCount <= 0) || (null == inventoryHitCount)))
			{
				isSellable = false;
			}
		}
