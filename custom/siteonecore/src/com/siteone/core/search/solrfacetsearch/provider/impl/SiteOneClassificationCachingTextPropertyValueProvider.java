 ...
 *
 */
public class SiteOneClassificationCachingTextPropertyValueProvider extends CommerceClassificationPropertyValueProvider
{
			...
			final Object model) throws FieldValueProviderException
	{
		if (model instanceof ProductModel)
		{
			...
			if (!cache.hasIndexedPropertyName(indexedProperty.getValueProviderParameter()))
			{
			...
			final List<ClassAttributeAssignment> indexPropertyAssignments = cache
					.getIndexedPropertyAssignments(indexedProperty.getValueProviderParameter());

			if (cache.haveAllProperties())
			{

				//process all cached FeatureContainer against the current property's assigments
			...
			{
				//first time around, we don't have all property assigments yet
	...
	private void cacheIndexedPropertyAssigments(final IndexedProperty indexedProperty) {
		...
		this.threadLocalCache.get().addIndexedProperty(indexedProperty.getValueProviderParameter(), classAttributeAssignmentList);
			...
			final List<ClassAttributeAssignment> assignments)
	{
		...
		final Collection<VariantProductModel> variants = baseProductModel.getVariants();
