 ...
 *
 */
public class ListCategoryCodeValueProvider extends CategoryCodeValueProvider
{
				...
				for (final LanguageModel language : languages)
				{
					for (final CategoryModel category : categories)
					{
						if (!category.getCode().startsWith(displayRootCategoryCode)
								&& !salesRootCategoryCode.equalsIgnoreCase(category.getCode()) && category.getCode().length() <= 6)
						{
