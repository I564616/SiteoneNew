 ...
 *
 */
public class SiteOneProductQuantityLevelTranslator extends AbstractValueTranslator
{
	private static final Logger LOG = Logger.getLogger(SiteOneProductQuantityLevelTranslator.class);
	...
	public Object importValue(final String cellValue, final Item orderEntry) throws JaloInvalidParameterException
	{
		...
		final int quantityLevel = (int) Double.parseDouble(splitValues[1]);
		final String division = splitValues[2];
		final String isCoreSku = splitValues[3];
		...
		try
		{
			...
			{
				productService.getProductForCode(catalogVersionCA, productCode);
			}
		}
		...
		if (null != productModel && isCoreSku.equalsIgnoreCase("0") && productModel.getProductType().equalsIgnoreCase(NURSERY))
		{
			return 0;
		}
