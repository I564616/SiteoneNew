...
<div style="background-image: url(${media.url});background-size:cover;"> 
<div class="siteone-4H">
	<c:forEach var="textBanner" items="${bannerList}">
	<c:url value="${textBanner.buttonURL}" var="encodedUrl" />
	<c:choose>
	<c:when test="${not empty textBanner.buttonLabel}">
		<div class="col-md-6 white-border">
			<div class="text-container">
				<div onclick="location.href='${encodedUrl}';" class="pointer outer-link">
					...
					<a href="${encodedUrl}" class="btn btn-primary">${textBanner.buttonLabel}</a>
				</div>
			</div>
		</div>
	</c:when>
	<c:otherwise>
		...
		<div class="col-md-3 text-center white-border">
			<div class="text-container">
				<div onclick="location.href='${encodedUrl}';" class="pointer outer-link">
					<h2>${textBanner.title}</h2>
					<br>
