...
<!-- New Popup -->
<div class="shipToInvoicePagePopup">
<div class="col-md-12">
<div class="row">
...
<hr class="visible-xs" />
 ...
 <spring:theme code="shiptosearch.placeholder" var="shiptosearchPlaceholder" />   
  <input type="text" id="search-ship-to-popup-invoice-new" name="searchParam" value="${searchParam}" onblur="ACC.invoicepage.invShipToPopUpOnblur()" onfocus="this.placeholder = ''" onblur="this.placeholder = '${shiptosearchPlaceholder}'"
            ...
            <c:when test="${not empty searchPageData.results}">
            	<c:if test="${not empty searchPageData.results}">
	                <div class="shipto-pagination pagination-hidden hidden-xs">
	                    <div class="account-orderhistory-pagination-invoice">
	                ...
	                <hr/>
	                <div class="black-title-txt f-s-20-xs-px f-w-500 font-Geogrotesque m-y-10">Sort By</div>
	                <div class="black-title-txt f-s-18-xs-px f-w-400 flex justify-between m-y-10">
	                ...
	                <hr/>
	                 <div class="black-title-txt f-s-20-xs-px f-w-500 font-Geogrotesque m-b-20-xs popup-boxshadow">Select Ship-to</div>
	                </div>
                </c:if>
                ...
                <div class="cl"></div>
                    ...
                    <c:forEach items="${searchPageData.results}" var="result">
                     ...
                     <spring:url value="/my-account/invoices/${result.displayId}${currentDivisionId}?invoiceShiptos=${fn:escapeXml(result.displayId)}+${fn:escapeXml(result.name)}&viewtype=All&searchParam=&dateFrom=${dateFrom}&dateTo=${dateTo}&pagesize=25&listOfShipTos=${fn:escapeXml(result.displayId)}+${fn:escapeXml(result.name)}&sort=byDate"
			         var="viewShipToDetailsUrlInvoice" htmlEscape="false">
			      </spring:url>
			      <div class="b-l-grey b-r-gray m-l-0 m-r-0 row shipTo-border shipTopopup-row hidden-xs">
			      	<div class="col-xs-12 col-md-4 col-sm-4 zeropadding">
				         ...
				         <div class="responsive-table-cell col-md-12 col-sm-12 col-xs-6 zeropadding">
				           <a class="bold-text text-green f-s-14" href="${viewShipToDetailsUrlInvoice}" id="orderPopup" data-name="${fn:escapeXml(result.name)}" data-index="${fn:escapeXml(result.displayId)}">${fn:escapeXml(result.displayId)}</a>
			         ...
			         <div class="col-xs-12 col-md-4 col-sm-4 zeropadding">
				         ...
				         <a class="bold-text f-s-14 text-gray" href="${viewShipToDetailsUrlInvoice}">${fn:escapeXml(result.name)}</a>
				         <input type="hidden" value="${fn:escapeXml(result.displayId)}" id="ship-toname"/>
				         <input type="hidden" value="${fn:escapeXml(result.displayId)}" id="ship-toId"/>
				         </div>
			         </div>
			      ...
			      <!----For Mobile---->
			       <div class="f-s-18-xs-px f-w-400 flex justify-between m-y-10 hidden-lg hidden-md">
	                ...
	                <span class="colored-radio"><input type="radio" class="shipToURLRadio" onclick="ACC.invoicepage.invshipToURLRadio()" name="shiptoURL" value="${viewShipToDetailsUrlInvoice}"></span>
	                </div>

			 </c:forEach> 

                              <div class="account-orderhistory-pagination-invoice sorting-hidden">
                        <nav:pagination top="false" supportShowPaged="${isShowPageAllowed}" supportShowAll="${isShowAllAllowed}"
                         ...
                         <c:otherwise>
			              ...
			              <div class="row">
			              <div class="col-xs-6">
							<input type="button" value="Clear All" class="btn btn-default full-width" onclick="ACC.invoicepage.invmoreShipToClear()"/>
							</div>
							<div class="col-xs-6">
							<input type="button" class="btn btn-primary full-width" onclick="ACC.invoicepage.invmoreShipToSearch()" value="Search">
							</div>
			              </div>
			              </div>
