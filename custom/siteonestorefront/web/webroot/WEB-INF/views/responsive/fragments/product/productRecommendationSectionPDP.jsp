...
{
"recommendedProductsLayer1":"<spring:escapeBody javaScriptEscape="true">
<c:if test="${algonomyRecommendationEnabled}">	
	<c:if test="${not empty recommendedProducts and recommendedProducts.size() gt 1}">
		<div class="feature-carousel prod-recom-layer-1-pdp">
			<div class="row"> 	
				<div class="col-xs-12 feature-carousel-products">
					...
					<h3 class="text-center margin20 black-title"><c:out value="${recommendationTitle}" /></h3>
						<div class="facet js-facet" style="overflow:auto">
							<div id="recommendedProductBannerrr1" class="facet__list js-facet-list">
							...
							<c:set var="noi" value="${(recommendedProducts.size()/3)+1}"/>
								...
								<c:forEach items="${recommendedProducts}" var="product" varStatus="status"><div class="category-tile-item"><product:featureProductLister product="${product}" isrc="${isrc}" loop="${status.index}"/></div></c:forEach>
								 <div class="category-tile-prevrr1 active">
								 ...
								 <div class="category-tile-nextrr1 active">
...
"recommendedProductsLayer2":"<spring:escapeBody javaScriptEscape="true">
<c:if test="${algonomyRecommendationEnabled}">	
	<c:if test="${not empty recommendedProductsrr2}">
		<div class="feature-carousel">
			<div class="row"> 	
				<div class="col-xs-12 padding0 feature-carousel-products">
					<div class="category-tiles product__facet js-product-facet">
					<h3 class="text-center margin20 black-title"><c:out value="${recommendationTitlerr2}" /></h3>
						<div class="facet js-facet" style="overflow:auto">
							<div id="recommendedProductBanner" class="facet__list js-facet-list">
							<input type="hidden" id="carouselProduct" value="${recommendedProductsrr2.size()}"/>
							<c:if test="${recommendedProductsrr2.size()%3==0}">
								<c:set var="noi" value="${recommendedProductsrr2.size()/3}"/>
							</c:if>
							<c:if test="${recommendedProductsrr2.size()%3!=0}">
							<c:set var="noi" value="${(recommendedProductsrr2.size()/3)+1}"/>
							</c:if>
		 						<c:set var="isrc" value="?isrc=featured"/>
								<c:forEach items="${recommendedProductsrr2}" var="product" varStatus="status"><div class="category-tile-item"><product:featureProductLister product="${product}" isrc="${isrc}" loop="${status.index}"/></div></c:forEach>
