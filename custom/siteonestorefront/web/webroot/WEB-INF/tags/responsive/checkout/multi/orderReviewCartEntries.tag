...
<jsp:useBean id="pdlist" class="java.util.ArrayList"/>
<jsp:useBean id="slist" class="java.util.ArrayList"/>
...
<c:if test="${empty splitProductType}">
...
<c:if test="${not empty splitProductType}">
	<c:if test="${splitProductType eq  'pickupAndDeliveryEntries'}">
		<c:forEach items="${cartData.pickupAndDeliveryEntries}" var="entry1" varStatus="loop">
			<c:set var="splitPickupEntries" value="${pdlist.add(entry1)}"/>
		</c:forEach>
	</c:if>
	<c:if test="${splitProductType eq  'shippingOnlyEntries'}">
		<c:forEach items="${cartData.shippingOnlyEntries}" var="entry1" varStatus="loop">
			<c:set var="splitShppingEntries" value="${slist.add(entry1)}"/>
		</c:forEach>
	</c:if>
</c:if>

<c:if test="${not empty list}">
	<multi-checkout:orderReviewCartEntriesSort entries="${list}" fulfillmentType="${cartData.orderType}" loopIndex="0"/>
</c:if>
<c:if test="${not empty list1}">
	<multi-checkout:orderReviewCartEntriesSort entries="${list1}" fulfillmentType="${cartData.orderType}" loopIndex="0"/> 
</c:if>
<c:if test="${not empty list2}">
	<multi-checkout:orderReviewCartEntriesSort entries="${list2}" fulfillmentType="${cartData.orderType}" loopIndex="0"/> 
</c:if>
<c:if test="${not empty slist}">
	<multi-checkout:orderReviewCartEntriesSort entries="${slist}" fulfillmentType="${cartData.orderType}" loopIndex="0"/> 
</c:if>
<c:if test="${not empty pdlist}">
	<multi-checkout:orderReviewCartEntriesSort entries="${pdlist}" fulfillmentType="${cartData.orderType}" loopIndex="0"/> 
</c:if>
</div>
</div>
</div>
