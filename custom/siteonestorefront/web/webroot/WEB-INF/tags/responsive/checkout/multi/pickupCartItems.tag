	...
	<li class="checkout-order-summary-list-heading">
		...
		<div class="address">
			...
			<c:if test="${ not empty groupData.deliveryPointOfService.address.phone }">
				<a class="tel-phone" href="tel:${fn:escapeXml(groupData.deliveryPointOfService.address.phone)}">${fn:escapeXml(groupData.deliveryPointOfService.address.phone)}</a>
			</c:if>
		</div>
	</li>
</c:if>
...
<ul class="item__list">
    <li class="hidden-xs hidden-sm">
     <ul class="item__list--header">
     	<li><spring:theme code="pickupCartItems.order" /></li>
     </ul>
    </li>
</ul>
<ul class="item__list">
        <multi-checkout:orderReviewCartEntries cartData="${cartData}"/>
</ul>