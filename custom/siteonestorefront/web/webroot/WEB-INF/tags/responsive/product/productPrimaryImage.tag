...
<img class="width-100-pe" src="${primaryImage.url}" alt="${fn:escapeXml(primaryImage.altText)}" title="${fn:escapeXml(primaryImage.altText)}"/>
			</c:when>
			<c:otherwise>
<img class="width-100-pe" src="${primaryImage.url}" alt="${fn:escapeXml(product.productShortDesc)}" title="${fn:escapeXml(product.productShortDesc)}"/>
			</c:otherwise>
		</c:choose>
	</c:when>
	<c:otherwise>
