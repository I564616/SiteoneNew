...
<div class="product-item-box produc-recomm-item">	
		...
		<sec:authorize access="hasAnyRole('ROLE_ANONYMOUS')">
			<c:choose>
				...
				<c:otherwise> <%--softlogin new code anonymous--%>
					...
					<div class="row">
						<div class="col-md-12  carousel-login-btn-wrapper">
							<c:if test="${product.inventoryFlag ne true }">
							<c:choose><c:when test="${hideCSP ne true}">
							<a href="<c:url value="/login"/>" class="logInToSeeYourPrice signInOverlay carousel-login-btn js-click-tracking"><spring:theme code="text.product.logInToSeeYourPricetext"/></a>
							</c:when>
							<c:otherwise>
...
<c:choose>
		...
		<c:set var="product" value="${product}" scope="request"/>
			...
			<product:recommendAddToCart product="${product}"/>
</c:otherwise>
</c:choose>