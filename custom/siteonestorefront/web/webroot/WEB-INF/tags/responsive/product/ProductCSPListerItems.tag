		...
		<c:choose>
			...
			<c:when test="${not empty product.sellableUomsCount and ((cmsPage.uid eq 'searchGrid') || (cmsPage.uid eq 'productGrid'))}">
				<c:choose>
					...
					<c:otherwise>
						<span class="csp-price-format"><format:cspprice priceData="${product.customerPrice}"/><c:if test="${sellableUomListLen==1}"><span class="check_price"> / ${uomDescription}</span></c:if></span>
					</c:otherwise>
				</c:choose>
			</c:when>
