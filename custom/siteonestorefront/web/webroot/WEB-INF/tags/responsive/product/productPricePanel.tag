...
<%@ attribute name="buyItAgainUOM" required="false" type="java.lang.String" %>
<%@ attribute name="isBuyItAgainUOM" required="false" type="java.lang.Boolean" %>
...
<c:forEach items="${product.sellableUoms}" var="sellableUom2">
	<c:if test="${product.hideUom eq true && sellableUom2.hideUOMOnline eq false}">
		...
		<c:set var="newuomDescription" value="${isBuyItAgainUOM eq true?buyItAgainUOM:sellableUom2.inventoryUOMDesc}"/>
		<c:set var="newuomMeasure" value="${isBuyItAgainUOM eq true?buyItAgainUOM: sellableUom2.measure}"/>
	</c:if>
</c:forEach>
<c:choose>
					...
					<c:choose>
						<c:when test="${not empty product.price}">
								<c:forEach items="${product.sellableUoms}" var="sellableUom">
									<c:set var="uomDescription" value="${isBuyItAgainUOM eq true?buyItAgainUOM:sellableUom.inventoryUOMDesc}"/>
									<c:set var="uomMeasure" value="${isBuyItAgainUOM eq true?buyItAgainUOM:sellableUom.measure}"/>
								 </c:forEach>
