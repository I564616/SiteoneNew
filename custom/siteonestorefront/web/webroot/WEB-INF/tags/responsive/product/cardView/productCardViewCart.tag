...
<input type="hidden" class="isStockNearByPLP"  value="${isStockNearByPLP}">
	...
	<c:choose>
        <c:when test="${(isAnonymous and not product.multidimensional and (hideList eq true || empty product.price ) and hideCSP ne true) or (isAnonymous and isGuestCheckoutEnabled eq false)}">
				<button type="submit" data-prod-code="${product.code}" class="btn btn-primary btn-block plp-loginToBuy js-login-to-buy"><spring:theme code="login.to.buy" /></button>
				<product:listaddtocartparametersPLP sellableUomListLen="${sellableUomListLen}" addIsMyStoreProduct="${addIsMyStoreProduct}" onlyHubStoreAvailability="${onlyHubStoreAvailability}" addIsProductSellable="${addIsProductSellable}" isSellableInventoryHit="${isSellableInventoryHit}"/>
			</c:when>
			
			<c:otherwise>
    <form:form id="addToCartForm${product.code}" action="${addToCartUrl}" method="post" class="add_to_cart_form">
		<product:listaddtocartparametersPLP sellableUomListLen="${sellableUomListLen}" addIsMyStoreProduct="${addIsMyStoreProduct}" onlyHubStoreAvailability="${onlyHubStoreAvailability}" addIsProductSellable="${addIsProductSellable}" isSellableInventoryHit="${isSellableInventoryHit}"/>
	<c:if test="${(not product.multidimensional) && (sellableUomListLen==1 || (product.sellableUomsCount>=1))}">
        <ycommerce:testId code="addToCartButton">
            <div class="row plp-addToCartSection">
                <div class="col-md-6 plp-qty-container flex-center col-xs-6 no-padding-rgt-xs no-padding-rgt-sm no-padding-rgt-md">
                    <div id="productPLPPostQtysection_${product.code}" class="flex-center plp-qtysection">
                    <button class="minusQty plp-qtyBtn flex-center border-rad-left" type="button" id="minusQty_${product.code}" onclick="ACC.product.plpPlusMinusQty('${product.code}', 'minus')"><common:minusIcon iconColor="#5A5B5D" /></button>
                    ...
                    <button class="plusQty plp-qtyBtn flex-center border-rad-right" type="button" id="plusQty_${product.code}" onclick="ACC.product.plpPlusMinusQty('${product.code}', 'plus')"><common:plusIcon iconColor="#5A5B5D" /></button>
                    </div>
                </div>
                <div class="card-atc-wrapper-section col-md-6 col-xs-6 pad-xs-lft-5 pad-sm-lft-5 pad-md-lft-5">
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <c:if test="${(empty product.sellableUomsCount || sellableUomListLen==1) && hideCSP eq true}">
                                <c:set var="isPLPATCClass" value="js-plp-uomqtyinterval" />
                                <c:set var="isPLPAriaDisabled" value="true" />
                            </c:if>
                        </c:otherwise>
                ...
                <div class="getAQuoteSection col-md-6 col-xs-6 pad-xs-lft-5 pad-sm-lft-5 pad-md-lft-5 hidden">
