...
<c:if test="${notPurchasable}">
	...
	<div class="${quotesFeatureSwitch ? 'quotesFeatureSwitchPLP' : ''} stock-section-moreontheway-section ${notPurchasable? ' hide ': ' '}">
		<div>
                ...
                <span id="stock5" class="stock-section-outofstock">
                    ...
                    <spring:theme code="stock.section.out-of-stock" />
						...
						<c:if test="${isMyStoreProduct}">
							<div id="regulatoryLicenceCheck_${product.code}">
								<div id="stock9" class="stock-section-notavailableonline-listview">
										<span>
											<spring:theme code="stock.section.not.available" /></br>
											<span class="check-branch" data-value="${product.code}" onclick="ACC.mystores.changebranchPLPPopupFn(${product.code},${product.code},false);"><spring:theme code="stock.section.check.branches" /></span>
										</span>
								</div>
							</div>
						</c:if>
					...
					<c:if test="${isMyStoreProduct && !isProductSellable && !onlyHubStoreAvailability}">
						<span id="regulatoryMessage_${product.code}">
							<div id="stock10" class="stock-section-notavailableonline-listview">
									<span>
										<spring:theme code="stock.section.not.available" /></br>
										<span class="check-branch" data-value="${product.code}" onclick="ACC.mystores.changebranchPLPPopupFn(${product.code},${product.code},false);"><spring:theme code="stock.section.check.branches" /></span>
									</span>
					...
					<c:if test="${!isMyStoreProduct && !onlyHubStoreAvailability}">
						<span id="regulatoryMessage_${product.code}">
							...
							<div id="stock11" class="stock-section-notavailableonline-listview">
										<span>
											<spring:theme code="stock.section.not.available" /></br>
											<span class="check-branch" data-value="${product.code}" onclick="ACC.mystores.changebranchPLPPopupFn(${product.code},${product.code},false);"><spring:theme code="stock.section.check.branches" /></span>
										</span>
											...
											<c:choose>
												...
												<c:otherwise>
													<c:choose>
														<c:when test="${hideList eq true && hideCSP eq true}">
																<div id="stock31"
																	class="stock-section-notavailableonline-listview">
																	<span> <spring:theme
																			code="stock.section.not.available" /></br> <span
																		class="check-branch" data-value="${product.code}"
																		onclick="ACC.mystores.changebranchPLPPopupFn(${product.code},${product.code},false);"><spring:theme
																				code="stock.section.check.branches" /></span>
																	</span>
																</div>
														</c:when>
														<c:otherwise>
