...
<input type="hidden" class="isStockNearByPLP"  value="${isStockNearByPLP}">
	...
	<c:choose>
        <c:when test="${(isAnonymous and not product.multidimensional and (hideList eq true || empty product.price ) and hideCSP ne true) or (isAnonymous and isGuestCheckoutEnabled eq false)}">
				<button type="submit" data-prod-code="${product.code}" class="btn btn-primary btn-block plp-loginToBuy js-login-to-buy ${currentLanguage.isocode eq 'es' ? 'logintobuy-spanish-text-plp' : ''}"><spring:theme code="login.to.buy" /></button>
    ...
    <form:form id="addToCartForm${product.code}" action="${addToCartUrl}" method="post" class="add_to_cart_form">
		<product:listaddtocartparametersPLP sellableUomListLen="${sellableUomListLen}" addIsMyStoreProduct="${addIsMyStoreProduct}" onlyHubStoreAvailability="${onlyHubStoreAvailability}" addIsProductSellable="${addIsProductSellable}" isSellableInventoryHit="${isSellableInventoryHit}"/>
        <input type="hidden" class="productPostPLPQty" id="productPostPLPQty_${product.code}" name="productPostPLPQty" value="1">
	<c:if test="${(not product.multidimensional) && (sellableUomListLen==1 || (product.sellableUomsCount>=1))}">
        <ycommerce:testId code="addToCartButton">
            <div class="row plp-addToCartSection">
                <div class="col-md-12 col-xs-6 list-atc-wrapper-section atcSectiondropdownicon ${currentLanguage.isocode eq 'es' ? 'atc-spanish-text-plp' : ''}">
                ...
                <div class="getAQuoteSection col-md-12 col-xs-6 pad-xs-lft-5 pad-sm-lft-5 pad-md-lft-5 hidden">
