...
<div class="global-alerts">	
		...
		<c:if test="${not empty accErrorMsgs}">
			<c:forEach items="${accErrorMsgs}" var="msg">
				<c:if test="${cmsPage.uid ne 'accountDashboardPage' && msg.code != 'partner.points.service.down.error'}">
				<div class="alert alert-box alert-dismissable alert-msg-section">
					<!-- <button class="close" aria-hidden="true" data-dismiss="alert" type="button">&times;</button> -->
					<div class="alert-msg-text"><spring:theme code="${msg.code}" arguments="${msg.attributes}"/></div>
					<div class="cl"></div>
				</div>
				</c:if>
			</c:forEach>
