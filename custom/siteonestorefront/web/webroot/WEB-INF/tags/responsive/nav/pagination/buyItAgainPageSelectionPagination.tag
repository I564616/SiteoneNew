...
<%@ attribute name="searchPageData" required="true" type="com.siteone.facade.BuyItAgainSearchPageData" %>
...
<c:set var="urlParameters" value="&pagesize=${searchPageData.pagination.pageSize}"/>
    ...
    <ul class="pagination">
        ...
        <c:if test="${hasPreviousPage}">
            <li class="pagination-prev">
                ...
                <a href="${previousPageUrl}${urlParameters}" rel="prev" class="previous-link">&#8592; <span class="hidden-xs hidden-sm"><spring:theme code="buyagainPageSelectionPagination.previous" /></span>
</a>
                </ycommerce:testId>
            </li>
        </c:if>
        ...
        <c:if test="${!hasPreviousPage}">
        <li class="pagination-prev disabled previous-link">&#8592; <span class="hidden-xs hidden-sm"><spring:theme code="pageSelectionPagination.previous" /></span>
</li>
        </c:if>
        ...
        <c:if test="${cmsPage.uid eq 'searchGrid' || cmsPage.uid eq 'productGrid' || cmsPage.uid eq 'search' || cmsPage.uid eq 'accountOrdersPage'}">
