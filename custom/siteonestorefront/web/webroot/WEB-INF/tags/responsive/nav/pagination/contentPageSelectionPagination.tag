...
<%@ attribute name="searchPageData" required="true" type="com.siteone.contentsearch.ContentSearchPageData" %>
        ...
        <c:if test="${hasPreviousPage}">
            <li class="pagination-prev">
                ...
                <ycommerce:testId code="searchResults_previousPage_link">
                  <a href="${previousPageUrl}&searchtype=content" rel="prev" class="previous-link">&#8592;&nbsp;Previous</a>
                </ycommerce:testId>
            </li>
        </c:if>

        <c:if test="${!hasPreviousPage}">
        <li class="pagination-prev disabled previous-link">&#8592;&nbsp;<spring:theme code="contentPageSelectionPagination.previous" />
</li>
        </c:if>
            ...
            <c:choose>
                <c:when test="${searchPageData.pagination.currentPage + 1 ne pageNumber}">
                    ...
                    <ycommerce:testId code="pageNumber_link">
                        <li><a class="${linkClass}" href="${pageNumberUrl}&searchtype=content">${pageNumber}</a></li>
                    </ycommerce:testId>
                </c:when>
                <c:otherwise>
        ...
        <c:if test="${hasNextPage}">
            <li class="pagination-next">
                ...
                <ycommerce:testId code="searchResults_nextPage_link">
                 <a href="${nextPageUrl}&searchtype=content" rel="next"><spring:theme code="contentPageSelectionPagination.next" />&nbsp;&#8594;</a>
                </ycommerce:testId>
            </li>
        </c:if>

        <c:if test="${!hasNextPage}">
        <li class="pagination-next disabled next-link"><spring:theme code="contentPageSelectionPagination.next" />&nbsp;&#8594;</li>
        </c:if>
    </ul>
