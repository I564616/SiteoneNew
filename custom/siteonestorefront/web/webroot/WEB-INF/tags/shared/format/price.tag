...
<c:when  test="${unitPrice}">
	...
	<c:when test="${priceData.value > 0}">
		<span class="black-title b-price add_price atc-price-analytics">$<fmt:formatNumber pattern="${unitpriceFormattedDigits}" value="${priceData.value}" /></span>
	</c:when>
	<c:otherwise>
		...
		<c:if test="${not displayFreeForZero}">
			<span class="black-title b-price atc-price-analytics">$<fmt:formatNumber pattern="${unitpriceFormattedDigits}" value="${priceData.value}" /></span>
		</c:if>
	</c:otherwise>
	</c:choose>
</c:when>
<c:otherwise>
	<c:choose>
	<c:when test="${priceData.value > 0}">
		<span class="black-title b-price add_price atc-price-analytics print-f-s-13 print-f-w-bold">
	...
	<c:otherwise>
		...
		<c:if test="${not displayFreeForZero}">
			<c:set var="formattedValue" value="${priceData.formattedValue}" />
			<c:set var="valueWithDollar" value="$${priceData.value}" />
			<span class="black-title b-price atc-price-analytics">${not empty formattedValue  ? formattedValue : valueWithDollar}</span>
			<input type="hidden" name="formattedValue" value="${formattedValue}" />
			<input type="hidden" name="valueWithDollar" value="${valueWithDollar}" />
		</c:if>
	</c:otherwise>
