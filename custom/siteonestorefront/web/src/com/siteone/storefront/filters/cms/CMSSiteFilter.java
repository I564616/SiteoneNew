 ...
 */
package com.siteone.storefront.filters.cms;
...
public class CMSSiteFilter extends OncePerRequestFilter implements CMSFilter
{
			...
			final FilterChain filterChain) throws ServletException, IOException
	{
		...
		final Cookie usernameCookie = WebUtils.getCookie(httpRequest, "j_username");
		String uid = null;
		if ((usernameCookie != null && usernameCookie.getValue() != null))
		{
			uid = URLDecoder.decode(usernameCookie.getValue(), "UTF-8");
		...
		if (null != customerModel && customerModel.getDisplayName() != null)
		{
			username = customerModel.getDisplayName();
		}

		if (null != username)
		{
			Integer hitNumber = (Integer) session.getAttribute("counterValue");
			if (username.equals(ANONYMOUS))
			{
				LOG.info("Anonymous user do nothing");
			}
			else
			{
				if (hitNumber == null)
				{
					...
					loginFlag = true;
					session.setAttribute("loginFlagModel", loginFlag);
				}
				else
				{
					hitNumber = Integer.valueOf(hitNumber.intValue() + 1);
				}
				session.setAttribute("counterValue", hitNumber);
			}
		}
