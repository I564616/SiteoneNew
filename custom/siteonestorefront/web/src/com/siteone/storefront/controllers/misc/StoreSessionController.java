...
import com.siteone.storefront.filters.StorefrontFilter;
...
public class StoreSessionController extends AbstractController
{
	...
	protected String getReturnRedirectUrlForUrlEncoding(final HttpServletRequest request, final String old, final String current)
	{
		...
		if (StringUtils.isNotBlank(originalReferer))
		{
			if (originalReferer.endsWith("/" + old) || originalReferer.endsWith("/" + current))
			{
				return REDIRECT_PREFIX + StringUtils.replace(originalReferer, "/" + old, "/" + current + "/");
			}
			else if (!StringUtils.contains(originalReferer, "/" + old + "/")
					&& !StringUtils.contains(originalReferer, "/" + current + "/"))
			{
				return REDIRECT_PREFIX + "https://"
						+ StringUtils.replaceOnce(StringUtils.substringAfter(originalReferer, "https://"), "/", "/" + current + "/");
			}
			return REDIRECT_PREFIX + StringUtils.replace(originalReferer, "/" + old + "/", "/" + current + "/");
		}
