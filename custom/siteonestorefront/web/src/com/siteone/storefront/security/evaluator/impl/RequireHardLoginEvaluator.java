 ...
 */
package com.siteone.storefront.security.evaluator.impl;
...
import com.siteone.storefront.security.evaluator.SecurityTraitEvaluator;
...
public class RequireHardLoginEvaluator implements SecurityTraitEvaluator
{
	...
	public boolean evaluate(final HttpServletRequest request, final HttpServletResponse response)
	{
		...
		if (null != getSessionService().getAttribute("guestUser"))
		{
			if (getSessionService().getAttribute("guestUser").toString().equalsIgnoreCase("guest"))
			{
				result = false;
			}
		}
