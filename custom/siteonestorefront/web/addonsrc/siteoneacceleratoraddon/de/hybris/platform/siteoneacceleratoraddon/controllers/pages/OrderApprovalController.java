...
import de.hybris.platform.siteoneacceleratoraddon.forms.OrderApprovalDecisionForm;
...
public class OrderApprovalController extends AbstractSearchPageController
{
	...
	{
		final PageableData pageableData = createPageableData(page, 25, sortCode, showMode);
		final SearchPageData<B2BOrderApprovalData> searchPageData = orderFacade
				.getPagedOrdersForApproval(new WorkflowActionType[]
		{ WorkflowActionType.START }, pageableData);
		populateModel(model, searchPageData, showMode);
		siteOneOrderFacade.updateFulfillmentAndPaymentInfoForApprovalOrders(searchPageData.getResults());
			...
			final Model model) throws CMSItemNotFoundException
	{
		try
		{
			...
			siteOneOrderFacade.setOrderApprovalDecision(b2bOrderApprovalData);
	...
	@PostMapping(path = "/order/updatePO")
	public @ResponseBody boolean updatePO
			(@RequestParam(value = "orderNumber", required = false) final String orderNumber,@RequestParam(value = "PO", required = false) final String PO,@RequestParam(value = "status", required = false) final String status, final HttpServletRequest request,
			final Model model) throws CMSItemNotFoundException // NOSONAR
	{
		
			return siteOneOrderFacade.updatePurchaseOrderNumber(orderNumber, PO,status);
