...
import com.siteone.facades.exceptions.*;
...
import de.hybris.platform.b2bcommercefacades.company.*;
...
import de.hybris.platform.siteoneorgaddon.breadcrumb.impl.MyCompanyBreadcrumbBuilder;
import de.hybris.platform.siteoneorgaddon.controllers.ControllerConstants;
import de.hybris.platform.siteoneorgaddon.forms.B2BBudgetForm;
import de.hybris.platform.siteoneorgaddon.forms.B2BCostCenterForm;
...
public class MyCompanyPageController extends AbstractSearchPageController
{
	...
	protected static final String ADD_USER_FULLPAGEPATH = "analytics.fullpath.add.user";
	protected static final String USER_DETAIL_FULLPAGEPATH = "analytics.fullpath.user.details";
	protected static final String EDIT_USER_FULLPAGEPATH = "analytics.fullpath.edit.user";
	
	private final Pattern PHONE_NUMBER_REGEX = Pattern.compile("\\d{3}?[-]\\d{3}?[-]\\d{4}");

	private final String USER_DETAILS_URL = REDIRECT_PREFIX + "/my-company/organization-management/manage-users/details/?unit=:unitId&user=%s";
	...
	@ModelAttribute("shipTos")
	public List<SelectOption> getshiptos()
	{
		final B2BUnitModel parent = ((SiteOneB2BUnitService) b2bUnitService).getParentUnitForCustomer();
		List<B2BUnitData> childs = ((SiteOneB2BUnitFacade) b2bUnitFacade).getShipTosForUnit(parent.getUid());
		return populateSelectBoxForB2BUnit(childs);
	}
	...
	protected String createUser(final Model model, final String unitId) throws CMSItemNotFoundException
	{
		if (!model.containsAttribute("b2BCustomerForm"))
		{
			...
			b2bCustomerForm.setRoles("b2bcustomergroup");
		...
		model.addAttribute("unitPayBillOnline",
				(((SiteOneB2BUnitFacade) b2bUnitFacade).getDefaultUnitForCurrentCustomer()).getPayBillOnline());
		getContentPageForLabelOrId(ORGANIZATION_MANAGEMENT_CMS_PAGE).setFullPagePath(getMessageSource().getMessage(ADD_USER_FULLPAGEPATH, null, getI18nService().getCurrentLocale()));
		storeCmsPageInModel(model, getContentPageForLabelOrId(ORGANIZATION_MANAGEMENT_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(ORGANIZATION_MANAGEMENT_CMS_PAGE));
		final List<Breadcrumb> breadcrumbs = myCompanyBreadcrumbBuilder.createManageUserBreadcrumb(unitId);
		breadcrumbs.add(new Breadcrumb("/my-company/organization-management/manage-users/" + unitId, getMessageSource().getMessage(
	...
	protected String createUser(final B2BCustomerForm b2BCustomerForm, final BindingResult bindingResult, final Model model,
			final RedirectAttributes redirectModel, final String unitId, HttpServletRequest request) throws CMSItemNotFoundException
	{

		if(validatePhoneNumber(b2BCustomerForm.getPhoneNumber()))
		{
		...
		if (bindingResult.hasErrors())
		{
			...
			return editUser(b2BCustomerForm.getUid(), model, unitId);
		}
		...
		b2bCustomerData.setDisplayUid(b2BCustomerForm.getEmail().toLowerCase());
		b2bCustomerData.setUnit(b2bUnitFacade.getUnitForUid(b2BCustomerForm.getParentB2BUnit()));
		b2bCustomerData.setRoles(Collections.singletonList(b2BCustomerForm.getRoles()));
		b2bCustomerData.setInvoicePermissions(b2BCustomerForm.isInvoicePermissions());
		b2bCustomerData.setPartnerProgramPermissions(b2BCustomerForm.isPartnerProgramPermissions());
		b2bCustomerData.setAccountOverviewForParent(b2BCustomerForm.isAccountOverviewForParent());
		b2bCustomerData.setAccountOverviewForShipTos(b2BCustomerForm.isAccountOverviewForShipTos());
		b2bCustomerData.setPayBillOnline(b2BCustomerForm.isPayBillOnline());
		...
		{
			if(b2BCustomerForm.isNeedsOrderApproval())
			{
				b2bCustomerData.setPlaceOrder(Boolean.TRUE);
				b2bCustomerData.setNeedsOrderApproval(Boolean.TRUE);
			}
			
			if(b2BCustomerForm.isPlaceOrder())
			{
				b2bCustomerData.setPlaceOrder(Boolean.TRUE);
				b2bCustomerData.setNeedsOrderApproval(Boolean.FALSE);
			}
			if(!(b2BCustomerForm.isPlaceOrder()) && null == b2bCustomerData.getPlaceOrder())
			{
				b2bCustomerData.setPlaceOrder(Boolean.FALSE);
				b2bCustomerData.setNeedsOrderApproval(Boolean.FALSE);
			}
		}
		else
		{
			b2bCustomerData.setPlaceOrder(b2BCustomerForm.isPlaceOrder());
		}
		
		
		b2bCustomerData.setContactNumber(b2BCustomerForm.getPhoneNumber());
		b2bCustomerData.setAssignedShipTos(b2BCustomerForm.getAssignedShipTo());
		b2bCustomerData.setEnableAddModifyDeliveryAddress(b2BCustomerForm.isEnableAddModifyDeliveryAddress());
		b2bCustomerData.setHomeBranch(((SiteOneCustomerFacade) customerFacade).getCurrentUserData());
		model.addAttribute(b2BCustomerForm);
		model.addAttribute("titleData", getUserFacade().getTitles());
		model.addAttribute("roles", populateRolesCheckBoxes(b2bUserGroupFacade.getUserGroups()));
		model.addAttribute("unitPayBillOnline",
		...
		breadcrumbs.add(new Breadcrumb("/my-company/organization-management/manage-user/"+unitId, getMessageSource().getMessage(
		...
		try
		{
			...
			GlobalMessages.addErrorMessage(model, "form.sytem.down.contact.error");
			return createUser(model, unitId);
		}catch(ContactNotCreatedOrUpdatedInUEException contactNotCreatedOrUpdatedInUEException) {
			LOG.error(contactNotCreatedOrUpdatedInUEException);
			GlobalMessages.addErrorMessage(model, "form.sytem.down.contact.error");
			return createUser(model, unitId);
		}catch(ContactNotCreatedOrUpdatedInOktaException contactNotCreatedOrUpdatedInOktaException) {
			LOG.error(contactNotCreatedOrUpdatedInOktaException);
			GlobalMessages.addErrorMessage(model, "form.sytem.down.contact.error");
			return createUser(model, unitId);
		}catch(ContactAlreadyPresentInUEException contactAlreadyPresentInUEException) {
			LOG.error(contactAlreadyPresentInUEException);
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER, "form.contact.already.exist.ue",
					new Object[]
			{ b2BCustomerForm.getEmail() });
			return createUser(model, unitId);
		}catch(ContactAlreadyPresentInBoomiException contactAlreadyPresentInBoomiException) {
			LOG.error(contactAlreadyPresentInBoomiException);
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER, "form.contact.already.exist.ue",
					new Object[]
			{ b2BCustomerForm.getEmail() });
			return createUser(model, unitId);
		}catch(DuplicateEmailException duplicateEmailException) {
			LOG.error("The uid of the model being stored already exists, could not save. Edit user instead.",duplicateEmailException);
			bindingResult.rejectValue("email", "text.manageuser.error.email.exists.title", new Object[]
					{ b2BCustomerForm.getEmail() }, "An account already exists for email address "+b2BCustomerForm.getEmail());
			GlobalMessages.addErrorMessage(model, "form.customer.already.exist");
		...
		catch (final ModelSavingException e) //NOSONAR
		{
			...
			{
				LOG.error("The uid of the model being stored already exists, could not save. Edit user instead.", e);
				bindingResult.rejectValue("email", "text.manageuser.error.email.exists.title", new Object[]
						{ b2BCustomerForm.getEmail() }, "An account already exists for email address "+b2BCustomerForm.getEmail());
				GlobalMessages.addErrorMessage(model, "form.global.error");
				model.addAttribute("b2BCustomerForm", b2BCustomerForm);
		...
		if(null != b2bCustomerData.getUid())
		{
			return String.format(USER_DETAILS_URL.replace(":unitId", unitId), urlEncode(b2bCustomerData.getUid()));
		}
		else
		{
			return String.format(USER_DETAILS_URL.replace(":unitId", unitId), urlEncode(b2bCustomerData.getEmail()));
		}
	
	}

	public String editUser(final String user, final Model model, final String unitId) throws CMSItemNotFoundException
	{
		List<Breadcrumb> breadcrumbs = myCompanyBreadcrumbBuilder.createManageUserDetailsBreadcrumb(user, unitId);
		CustomerData customerData = null;
		try
		{
			if(null != user)
			{
			customerData = b2bUserFacade.getCustomerForUid(user);
			}
		}catch(Exception e){
			LOG.error("No customer with uid found", e);
		}
		if (!model.containsAttribute("b2BCustomerForm"))
		{
			final B2BCustomerForm b2bCustomerForm = new B2BCustomerForm();
			...
			if(null != customerData) {
				...
				final Collection<String> roles = customerData.getRoles() != null ? customerData.getRoles() : new ArrayList<String>();
				if (roles.contains(B2BConstants.B2BADMINGROUP))
				{
					b2bCustomerForm.setRoles(B2BConstants.B2BADMINGROUP);
				}
				else
				{
					b2bCustomerForm.setRoles(B2BConstants.B2BCUSTOMERGROUP);
				}
				
				if(null != customerData.getInvoicePermissions()) 
				{
					b2bCustomerForm.setInvoicePermissions(customerData.getInvoicePermissions());
				}
				if(null != customerData.getPartnerProgramPermissions())
				{
					b2bCustomerForm.setPartnerProgramPermissions(customerData.getPartnerProgramPermissions());
				}
				if(null != customerData.getAccountOverviewForParent())
				{
					b2bCustomerForm.setAccountOverviewForParent(customerData.getAccountOverviewForParent());
				}
				if(null != customerData.getAccountOverviewForShipTos())
				{
					b2bCustomerForm.setAccountOverviewForShipTos(customerData.getAccountOverviewForShipTos());
				}
				if(null != customerData.getPayBillOnline())
				{
					b2bCustomerForm.setPayBillOnline(customerData.getPayBillOnline());
				}
				if(null != customerData.getPlaceOrder()) 
				{
					b2bCustomerForm.setPlaceOrder(customerData.getPlaceOrder());
				}
				if(null!= customerData.getLangPreference()){
					
					b2bCustomerForm.setLangPreference(customerData.getLangPreference().getName(Locale.ENGLISH));
					
				}
				if(null != customerData.getEnableAddModifyDeliveryAddress())
				{
					b2bCustomerForm.setEnableAddModifyDeliveryAddress(customerData.getEnableAddModifyDeliveryAddress());
				}
				if(null!=customerData.getNeedsOrderApproval())
            {
            b2bCustomerForm.setNeedsOrderApproval(customerData.getNeedsOrderApproval());
            }
				b2bCustomerForm.setPhoneNumber(customerData.getContactNumber());
				b2bCustomerForm.setAssignedShipTo(customerData.getAssignedShipTos());
				
			}
			
			model.addAttribute(b2bCustomerForm);
		}
		if(null != customerData && null != customerData.getUid())
		{
		model.addAttribute("parentUnitName", b2bUserFacade.getParentUnitForCustomer(customerData.getUid()).getName());
		}
		model.addAttribute("titleData", getUserFacade().getTitles());
		model.addAttribute("roles", populateRolesCheckBoxes(b2bUserGroupFacade.getUserGroups()));
		model.addAttribute("unitPayBillOnline",
		...
		getContentPageForLabelOrId(ORGANIZATION_MANAGEMENT_CMS_PAGE).setFullPagePath(getMessageSource().getMessage(EDIT_USER_FULLPAGEPATH, null, getI18nService().getCurrentLocale()));
		storeCmsPageInModel(model, getContentPageForLabelOrId(ORGANIZATION_MANAGEMENT_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(ORGANIZATION_MANAGEMENT_CMS_PAGE));

		if (model.containsAttribute("newUserEdit"))
		{
			breadcrumbs = myCompanyBreadcrumbBuilder.createManageUserBreadcrumb(unitId);
			breadcrumbs.add(new Breadcrumb("#", getMessageSource().getMessage("text.company.organizationManagement.createuser", null, getI18nService().getCurrentLocale()), null));
		}
		else
		{
			String url = "/my-company/organization-management/manage-users/edit?unit="+unitId+"&user=%s";
			...
			breadcrumbs.add(new Breadcrumb("#", getMessageSource().getMessage("text.company.manageusers.edit", new Object[]
			{firstName,lastName}, "Edit {0} User", getI18nService().getCurrentLocale()), null));
		}
		model.addAttribute("breadcrumbs", breadcrumbs);
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return ControllerConstants.Views.Pages.MyCompany.MyCompanyManageUserAddEditFormPage;
	...
	protected String editUser(final String user, final B2BCustomerForm b2BCustomerForm, final BindingResult bindingResult,
			final Model model, final RedirectAttributes redirectModel, final String unitId, HttpServletRequest request) throws CMSItemNotFoundException
	{
		...
		{
			String currentRole = b2BCustomerForm.getRoles() != null ? b2BCustomerForm.getRoles() : null;
			if(null != currentRole)
			{
   			if (!currentRole.contains(B2BConstants.B2BADMINGROUP))
   			{
   				GlobalMessages.addErrorMessage(model, "form.b2bcustomer.adminrole.error");
   				currentRole = B2BConstants.B2BADMINGROUP;
   				b2BCustomerForm.setRoles(currentRole);
   				model.addAttribute(b2BCustomerForm);
   				return editUser(b2BCustomerForm.getUid(), model, unitId);
   			}
   			else
		...
		b2bCustomerData.setUnit(b2bUnitFacade.getUnitForUid(b2BCustomerForm.getParentB2BUnit()));
		b2bCustomerData.setRoles(b2BCustomerForm.getRoles() != null ? Collections.singletonList(b2BCustomerForm.getRoles()) : Collections.<String> emptyList());
		b2bCustomerData.setInvoicePermissions(b2BCustomerForm.isInvoicePermissions());
		b2bCustomerData.setPartnerProgramPermissions(b2BCustomerForm.isPartnerProgramPermissions());
		b2bCustomerData.setAccountOverviewForParent(b2BCustomerForm.isAccountOverviewForParent());
		b2bCustomerData.setAccountOverviewForShipTos(b2BCustomerForm.isAccountOverviewForShipTos());
		...
		b2bCustomerData.setContactNumber(b2BCustomerForm.getPhoneNumber());
		b2bCustomerData.setEnableAddModifyDeliveryAddress(b2BCustomerForm.isEnableAddModifyDeliveryAddress());
		if (b2BCustomerForm.getLangPreference() != null)
		{
			if (b2BCustomerForm.getLangPreference().toLowerCase().startsWith("en"))
			{
				b2bCustomerData.setLangPreference(getI18nService().getLanguage("en"));
			}
			else
			{
				b2bCustomerData.setLangPreference(getI18nService().getLanguage("es"));
			}
		}
		else
		{
		...
		if (!b2bUserFacade.getCustomerForUid(b2BCustomerForm.getUid()).getUnit().getUid()
				.equalsIgnoreCase(b2BCustomerForm.getParentB2BUnit()))
		{
			b2bCustomerData.setVaultToken(null);
		}
		else
		{
			b2bCustomerData.setVaultToken(b2bUserFacade.getCustomerForUid(b2BCustomerForm.getUid()).getVaultToken());
		}
		b2bCustomerData.setAssignedShipTos(b2BCustomerForm.getAssignedShipTo() != null ? b2BCustomerForm.getAssignedShipTo() : Collections.<String> emptyList());
		model.addAttribute(b2BCustomerForm);
		model.addAttribute("titleData", getUserFacade().getTitles());
		model.addAttribute("roles", populateRolesCheckBoxes(b2bUserGroupFacade.getUserGroups()));
		model.addAttribute("unitPayBillOnline",
		...
		try
		{
			...
			LOG.error(contactNotCreatedOrUpdatedInUEException);
			GlobalMessages.addErrorMessage(model, "form.sytem.down.contact.ue.error");
			return createUser(model, unitId);
		}catch(ContactNotCreatedOrUpdatedInOktaException contactNotCreatedOrUpdatedInOktaException) {
			LOG.error(contactNotCreatedOrUpdatedInOktaException);
			GlobalMessages.addErrorMessage(model, "form.sytem.down.contact.error");
		...
		{
			LOG.error("Failed to edit user. Email already exists.", e);
	...
	protected String manageUserDetail(final String user, final Model model, final String unitId) throws CMSItemNotFoundException
	{
		final CustomerData customerData = b2bUserFacade.getCustomerForUid(user);
		model.addAttribute("customerData", customerData);
		getContentPageForLabelOrId(ORGANIZATION_MANAGEMENT_CMS_PAGE).setFullPagePath(getMessageSource().getMessage(USER_DETAIL_FULLPAGEPATH, null, getI18nService().getCurrentLocale()));
		storeCmsPageInModel(model, getContentPageForLabelOrId(ORGANIZATION_MANAGEMENT_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(ORGANIZATION_MANAGEMENT_CMS_PAGE));
		final List<Breadcrumb> breadcrumbs = myCompanyBreadcrumbBuilder.createManageUserDetailsBreadcrumb(user, unitId);
		model.addAttribute("breadcrumbs", breadcrumbs);
	...
	protected List<SelectOption> populateSelectBoxForB2BUnit(final List<B2BUnitData> unitData)
	{
		...
		{
			if(unit.getUid().equalsIgnoreCase(unit.getReportingOrganization().getUid()))
			{
				selectOptions.add(0,new SelectOption(unit.getUid(), unit.getName().concat(" (Main Account)")));
			}
			else
			{
	...
	protected List<SelectOption> populateRolesCheckBoxes(final List<String> roles)
	{
		...
		{
			if(data.equalsIgnoreCase("b2badmingroup") || data.equalsIgnoreCase("b2bcustomergroup"))
			{
	...
	{
		return getSiteConfigService().getInt("siteoneorgaddon.search.pageSize", 9);
	}
