 ...
 */
package de.hybris.platform.siteoneorgaddon.breadcrumb.impl;
...
public class MyCompanyBreadcrumbBuilder
{
	...
	public List<Breadcrumb> createManageUserBreadcrumb(final String unit)
	{
		final List<Breadcrumb> breadcrumbs = this.getBreadcrumbs(null);
		breadcrumbs.add(new Breadcrumb(MY_ACCOUNT_DASHBOARD_URL, messageSource.getMessage(
				"text.company.manageUsers", null, i18nService.getCurrentLocale()), null));
		return breadcrumbs;
	}


	public List<Breadcrumb> createManageUserDetailsBreadcrumb(final String uid, final String unit)
	{
		String url = "/my-company/organization-management/manage-users/?unit=" + unit;
		final List<Breadcrumb> breadcrumbs = createManageUserBreadcrumb(unit);
		breadcrumbs.add(new Breadcrumb(String.format(MY_COMPANY_MANAGE_USER_URL + "/" + unit,
