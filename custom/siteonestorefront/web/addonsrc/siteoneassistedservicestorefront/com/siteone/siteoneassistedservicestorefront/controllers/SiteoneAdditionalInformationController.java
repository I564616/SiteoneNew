...
import de.hybris.platform.assistedservicestorefront.controllers.AdditionalInformationController;
 ...
 */
public class SiteoneAdditionalInformationController extends AdditionalInformationController {
	...
	private static final String CUSTOMER_360_PAGE = "addon:/siteoneassistedservicestorefront/pages/customer360";
	...
	public String getCustomer360(final Model model, final HttpServletResponse response)
	{
		...
		List<Section> sectionList = additionalInformationFrameworkFacade.getSections();
		
		
        Predicate<Section> condition = section -> section.getId().startsWith("feedbackSection");
        sectionList.removeIf(condition);
        condition = section -> section.getId().startsWith("storeLocationsSection");
        sectionList.removeIf(condition);
		...
		@RequestParam("sectionId") final String sectionId, final HttpServletResponse response)
	{
		...
		super.getCustomer360Section(model, sectionId, response);
...
**/
		System.out.println("overridden method customer360Fragment");
		return super.getCustomer360Fragment(model, allRequestParams, response);
