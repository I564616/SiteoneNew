 ...
 */
package de.hybris.platform.siteonepunchoutaddon.interceptors;
...
public class PunchOutCartBeforeViewHandlerTest
{
	...
	public void changesViewForPunchOutUser() throws Exception
	{
		...
		Mockito.when(session.getAttribute(SiteonepunchoutaddonConstants.PUNCHOUT_USER)).thenReturn("myUser");

		viewHandler.beforeView(request, response, modelAndView);
		Assert.assertNotNull(modelAndView.getViewName());
		Assert.assertEquals(SiteonepunchoutaddonConstants.VIEW_PAGE_PREFIX + NEW_VIEW, modelAndView.getViewName());
	}
	...
	{
		Mockito.when(request.getSession()).thenReturn(session);
		Mockito.when(session.getAttribute(SiteonepunchoutaddonConstants.PUNCHOUT_USER)).thenReturn(null);
