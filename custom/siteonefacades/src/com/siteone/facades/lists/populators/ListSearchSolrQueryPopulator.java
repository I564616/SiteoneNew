 ...
 *
 */
public class ListSearchSolrQueryPopulator<INDEXED_PROPERTY_TYPE, INDEXED_TYPE_SORT_TYPE>
		extends SearchSolrQueryPopulator<INDEXED_PROPERTY_TYPE, INDEXED_TYPE_SORT_TYPE>
{
	private static final Logger LOG = Logger.getLogger(ListSearchSolrQueryPopulator.class);
	private static final String SOLR_WISHLIST_CODE = "sowishlist_string";
			...
			final SolrSearchRequest<FacetSearchConfig, IndexedType, INDEXED_PROPERTY_TYPE, SearchQuery, INDEXED_TYPE_SORT_TYPE> target)
	{
		...
		final String wishlistCode = sessionService.getAttribute("WishlistCode");
		LOG.error("search solr query code" + wishlistCode);
		final StringBuilder sb = new StringBuilder();
		if (null != wishlistCode)
		{
			sb.append(SOLR_WISHLIST_CODE).append(":\\/").append(wishlistCode);
		}
		//searchQuery.addFilterRawQuery(sb.toString());
		searchQuery.addFilterQuery(SOLR_WISHLIST_CODE, wishlistCode);
		...
		LOG.error(
				"search solr target " + searchQuery.getFilterQueries().toString() + " search user query " + searchQuery.getUserQuery()
						+ " free text " + source.getSearchQueryData().getFreeTextSearch() + " filter raw " + sb.toString());
			...
			final SearchQueryContext searchQueryContext, final String freeTextSearch)
	{
		...
		LOG.error("search solr legacy " + searchQuery.getQueries() + "free text " + freeTextSearch);
		return searchQuery;
	}
			...
			final SearchQueryContext searchQueryContext, final String freeTextSearch)
	{
		...
		LOG.error("search solr create " + searchQuery.getQueries());
		return searchQuery;
	}
	...
	{
		return getFacetSearchConfigService().getConfiguration("listSearchIndex");
	}
