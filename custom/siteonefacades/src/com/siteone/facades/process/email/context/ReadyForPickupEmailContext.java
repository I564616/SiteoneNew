...
{
	private static final Logger LOG = Logger.getLogger(ReadyForPickupEmailContext.class);
	...
	public void init(final ConsignmentProcessModel consignmentProcessModel, final EmailPageModel emailPageModel)
	{
		...
		final BaseSiteModel baseSite = getSite(consignmentProcessModel);
		if (baseSite == null)
		{
			LOG.error("Failed to lookup Site for BusinessProcess : ["+ consignmentProcessModel + "]");
		}
		else
		{
			put(COUNTRY_BASESITE_ID, baseSite.getUid());
		}
		storeSessionFacade.setCurrentBaseStore(consignmentProcessModel.getConsignment().getOrder().getPointOfService().getDivision().getUid());
