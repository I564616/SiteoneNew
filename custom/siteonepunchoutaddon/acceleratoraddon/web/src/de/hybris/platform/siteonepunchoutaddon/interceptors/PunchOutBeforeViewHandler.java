...
public class PunchOutBeforeViewHandler implements BeforeViewHandler
{
			...
			throws PunchOutException
	{
		...
		try
		{
			if (StringUtils.isNotBlank((String) request.getSession().getAttribute(SiteonepunchoutaddonConstants.PUNCHOUT_USER)))
			{
	...
	protected String getPunchoutView(String viewName)
	{
		...
		{
			viewName = SiteonepunchoutaddonConstants.VIEW_PAGE_PREFIX + viewMap.get(viewName).get(VIEW_NAME_MAP_KEY);
		}
		return viewName;
	}
