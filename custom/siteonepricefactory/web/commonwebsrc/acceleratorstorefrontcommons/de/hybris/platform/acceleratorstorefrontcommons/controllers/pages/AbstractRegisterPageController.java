...
public abstract class AbstractRegisterPageController extends AbstractPageController
{
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		{
			return handleDuplicateUidException(redirectModel);
		}
		...
		if (cookie != null)
		{
			try
			{
				...
				final List<AnonymousConsentData> anonymousConsentDataList = Arrays.asList(
						mapper.readValue(URLDecoder.decode(cookie.getValue(), StandardCharsets.UTF_8.displayName()),
								AnonymousConsentData[].class));
				anonymousConsentDataList.stream().filter(consentData -> CONSENT_GIVEN.equals(consentData.getConsentState())).forEach(
						consentData -> consentFacade.giveConsent(consentData.getTemplateCode(),
		...
		return handleRedirectInRegistration(anonymousCustomer, redirectModel);
	...
	private String handleDuplicateUidException(final RedirectAttributes redirectModel)
	{
	...
	private String handleRedirectInRegistration(final CustomerData anonymousCustomer, final RedirectAttributes redirectModel)
	{
