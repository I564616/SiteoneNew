...
public class SubprocessEndAction extends AbstractProceduralAction<ConsignmentProcessModel>
{
	...
	public void executeAction(final ConsignmentProcessModel process)
	{
		...
		try
		{
			...
			Thread.sleep((long) (rand.nextDouble() * 2000));
		}
		...
		getBusinessProcessService().triggerEvent(
				process.getParentProcess().getCode() + "_" + SiteoneFulfilmentProcessConstants.CONSIGNMENT_SUBPROCESS_END_EVENT_NAME);
		LOG.info(PROCESS_MSG + process.getCode() + " fired event "
				+ SiteoneFulfilmentProcessConstants.CONSIGNMENT_SUBPROCESS_END_EVENT_NAME);
	}
}
